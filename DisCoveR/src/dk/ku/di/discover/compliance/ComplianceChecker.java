...
public class ComplianceChecker {
	...
	public static ComplianceResult CheckCompliance(DCRGraph dcrGraph, DCRGraph dcrGraphFilter, XLog log,
			HashMap<String, Boolean> labelling) {
		...
		for (XTrace trace : log) {
			...
			for (XEvent event : trace) {
				if (event != null) {
					...
					// countEvents++;
					// For Amine: here you want to add your code for mapping Log activities to DCR
					// activities.
			...
			if (actualResult)
				...
				result.AddResult(new TraceComplianceResult(traceId, expectedResult, actualResult,
						dcrGraph.whyInvalidRunOpenWorld(t)));
		}
