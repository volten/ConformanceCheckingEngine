...
public class DisCoveR {
	...
	public static void main(String[] args) throws Exception {
		...
		if (args.length == 0) {
			...
			System.out.println("");
			System.out.println(
					"When using the third option, test logs should have exactly the same name as the training logs, and each test folder should contain a log for each log in the input folder.");
			...
			for (final File fileEntry : fInputFolder.listFiles()) {
				mine(sInputFolder + "/" + fileEntry.getName(),
						sOutputFolder + "/" + fileEntry.getName().replace(".xes", ".txt"));
			}
		}
			...
			for (final File fileEntry : fInputFolder.listFiles()) {
				mineAndTest(sInputFolder + "/" + fileEntry.getName(),
						sOutputFolder + "/" + fileEntry.getName().replace(".xes", ".txt"), sVerficiationFolders,
						fileEntry.getName(), sOutputFolder);
			}
	...
	public static void minePDC(String location, String output, boolean light) throws Exception {
		System.out.println("=== " + location + " => " + output + " ===");
		long startTime = System.currentTimeMillis();
		System.out.print("Loading XES... ");
		XLog log = XLogHelper.readFile(location);
	...
	public static void mine(String location, String output) throws Exception {
		System.out.println("=== " + location + " => " + output + " ===");
		long startTime = System.currentTimeMillis();
		System.out.print("Loading XES... ");
		XLog log = XLogHelper.readFile(location);
	...
	public static void mineAndTest(String location, String output, String[] verification, String logName,
	...
	private static void test(BitParNekLogAbstractions helper, BitDCRGraph g, String s, String output) {
		...
		XLog log;
		try {
			log = XLogHelper.readFile(s);

			BitParNek disco = new BitParNek();

			for (XTrace trace : log) {
				String name = XLogHelper.getName(trace);
